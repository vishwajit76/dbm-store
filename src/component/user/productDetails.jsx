...
const ProductDetails = ({ onClose, product, cartDrawer }) => {
  ...
  const handleVariationClick = (index = 0, id) => {
    const selectedVariation = productData.variations
      ? productData.variations.find((item) => id === item._id)
      : productData.variations.find((item, idx) => index === idx);
